...
Root Directory を **`api`** にしたとき、エントリは **`pyproject.toml` の `[project.scripts] app = "main:app"`** で指定します（`main.py` だけでは Vercel の FastAPI 自動検出の対象外のため）。ルートの `vercel.json` は不要です（誤った `functions` パターンでビルドが落ちるのを避けるため）。タイムアウトを延ばす場合は Vercel の **Project Settings → Functions** で調整してください。
...
3. デプロイ後に表示される URL（例: `https://xxx.vercel.app`）を Flutter のビルド時に渡す。あわせて Supabase の URL / anon キーも `.env` に書く。
